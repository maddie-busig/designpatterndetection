...
def main():
    if len(sys.argv) < 4:
        print "Usage: build_corpus IN_CORPUS_DIR OUT_CORPUS_DIR --names PROJECT_NAMES..."
        print "Usage: build_corpus IN_CORPUS_DIR OUT_CORPUS_DIR PROJECT_LIST_FILE WHITELIST_FILE"
        return -1
    ...
    # Delete all files that are not .java
    for project_name in project_names:
        out_proj_dir = os.path.join(out_corpus_dir, project_name)
        for root, dirs, files in os.walk(out_proj_dir):
